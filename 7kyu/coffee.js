...
//string wprd coffee, can repeat
// uppercase just the words COFFEE
//"Sweet coffee." -> "Sweet COFFEE."
//Use replace all method

function coffee(str) {
    return str.replaceAll('coffee', 'COFFEE').replace('coFFEE', 'COFFEE').replace('Coffee', 'COFFEE')
  }
...
function coffee(str) {
    ...
    return arr.map(item=> {
      if(item.toLowerCase().includes('coffee')){
        return item.replace(/[coffee]/g, item => obj[item]? obj[item] : item)
      }else{
        return item
      }
    }).join(' ')
